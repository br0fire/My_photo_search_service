...
# Quick Face Recognition Training Pipeline
#
# This script runs the entire face recognition pipeline with quick training:
# 1. Downloads data from Roboflow
# 2. Trains a face recognition model with frozen backbone (faster)
#
# Usage:
...
if [ -z "$1" ]; then
  ...
  echo "Usage: ./run_quick_training.sh YOUR_ROBOFLOW_API_KEY"
  exit 1
fi
...
LOG_FILE="quick_training_${TIMESTAMP}.log"

echo "Starting quick face recognition training pipeline at $(date)" | tee -a "$LOG_FILE"
echo "====================================================" | tee -a "$LOG_FILE"
...
# Step 2: Train the model with quick settings (frozen backbone)
echo "Step 2: Training the face recognition model (quick version)..." | tee -a "$LOG_FILE"
...
echo "====================================================" | tee -a "$LOG_FILE"
echo "Quick face recognition training pipeline completed successfully at $(date)" | tee -a "$LOG_FILE"
